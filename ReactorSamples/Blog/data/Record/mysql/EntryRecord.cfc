...
<cfcomponent hint="I am the custom Record object for the  table.  I am generated, but not overwritten if I exist.  You are safe to edit me."
	...
	<cffunction name="getAverageRating" access="public" hint="I return the average rating for this entry" output="false" returntype="numeric">
		...
		<cfquery name="qRating" datasource="#_getConfig().getDsn()#" cachedwithin="#CreateTimespan(0,0,0,5)#">
			SELECT getAverageRating(#getEntryId()#) avgRating
		</cfquery>
		
		<cfreturn Val(qRating.avgRating) />
	</cffunction>
	
	<cffunction name="setDisableComments" access="public" output="false" returntype="void">
		<cfargument name="disableComments" hint="I indicate if comments are disabled." required="yes" type="boolean" />
		<cfset _getTo().disableComments = Iif(arguments.disableComments, DE(1), DE(0)) />
	</cffunction>
	<cffunction name="getDisableComments" access="public" output="false" returntype="boolean">
