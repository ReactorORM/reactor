...
&lt;cfcomponent hint="I am the base Gateway object for the <xsl:value-of select="table/@name"/> table.  I am generated.  DO NOT EDIT ME."
	extends="reactor.core.abstractGateway" &gt;
	...
	<xsl:choose>
	<xsl:when test="table/@name = table/@baseTable"><xsl:text>	</xsl:text>
		<xsl:for-each select="table/columns/column">&lt;cfargument name="<xsl:value-of select="@name" />" hint="If provided, I match the provided value to the <xsl:value-of select="@name" /> column." required="no" type="string" /&gt;
		</xsl:for-each>
		&lt;cfset var qGet = 0 /&gt;
		...
		&lt;cfquery name="qGet" datasource="#getDsn()#"&gt;
			...
			WHERE 1=1
				<xsl:for-each select="table/columns/column">&lt;cfif IsDefined('arguments.<xsl:value-of select="@name" />')&gt;
					AND [<xsl:value-of select="@name" />] = &lt;cfqueryparam<xsl:choose>
	...
	<xsl:when test="table/@name != table/@baseTable"><xsl:text>	</xsl:text>
		<xsl:for-each select="table/baseTableColumns/baseTableColumn[@overridden = 'false']">&lt;cfargument name="<xsl:value-of select="@name" />" hint="If provided, I match the provided value to the <xsl:value-of select="@name" /> column in the <xsl:value-of select="../../@baseTable" /> table." required="no" type="string" /&gt;
		</xsl:for-each>
		<xsl:for-each select="table/columns/column">&lt;cfargument name="<xsl:value-of select="@name" />" hint="If provided, I match the provided value to the <xsl:value-of select="@name" /> column in the <xsl:value-of select="../../@name" /> table." required="no" type="string" /&gt;
		...
		&lt;cfquery name="qGet" datasource="#getDsn()#"&gt;
			SELECT 
				<xsl:for-each select="table/baseTableColumns/baseTableColumn[@overridden = 'false']">[<xsl:value-of select="../../@baseTable" />].[<xsl:value-of select="@name" />],
				</xsl:for-each>
